	...
	<div>
		...
		<section>
				...
				<el-col :span='24'>
					<el-form :inline='true' :model='filters'>
						...
						<el-form-item label='状态：'>
							<el-select v-model='filters.status' class='ipt_sta'>
								<el-option v-for='(item,index) in staData'
									:key='index'
									:label='item.name'
			...
			<el-table :data='users' highlight-current-row @selection-change="selsChange" style="width: 100%;" v-loading='loading'>
				...
				<el-table-column prop='email' width='150' align='center' label='租户标识'>
					<template slot-scope='scope'>
						<span class="tem_span" @click="handleSee(scope.$index, scope.row)">{{scope.row.name}}</span>
					</template>
				</el-table-column>
				...
				<el-table-column prop='description' width='100' align='center' label='备注'></el-table-column>
				<el-table-column  width='300' label='操作' align='center'>
					<template slot-scope='scope'>
						<el-button type='info'size='small' @click='handleSta(scope.$index, scope.row)' class='cusSta'>{{scope.row.usableBtnText}}</el-button>
...
//				console.log(`当前页数是:${val}`)
			...
			handleDel(index,row){
				...
				.then(() => {
					this.$ajax.del('/tenant/del_tenant/'+row.id+'?token='+this.token)
					.then(res => {
						console.log(res)
						if(res.status=='200'){
							if(res.data.status=='0'){
			...
			batchRemove:function(rows){
				...
				.then(() => {
					...
					this.$ajax.del('/tenant/del_tenants'+'?token='+this.token,para)
					.then(res => {
						console.log(res);
						if(res.status=='200'){
							if(res.data.status=='0'){
		...
		//			console.log('数据不是空');
		    ...
		    exportData:function(params){
		    	...
		    	this.$ajax.get('/tenant/tenants'+'?token='+this.token,params)
		    	.then(res => {
			...
			export2Excel(){
				...
				require.ensure([],() => {
					...
					const tHeader=['客户标识','租户联系人','邮箱','手机号','公司名称','公司 类型','公司电话','详细地址','描述信息'];
					const filterVal=['name','contact','email','mobile','company_name','company_type','phone','extra','description'];
