	...
	<div>
		...
		<section>
			...
			<el-table :data='users'highlight-current-row @selection-change="selsChange" style='width: 100%;' v-loading='loading'>
				...
				<el-table-column prop='monitoring' label='链路检测' align='center' width='100'>
				</el-table-column>
			...
			<el-dialog :title='textMap[dialogStatus]':visible.sync='dialogFormVisible':close-on-click-modal="false" v-loading='editLoading'>
				<el-form :model="editForm" label-width='80px'ref='editForm' label-position='left' >
					...
					<el-form-item label='链路ID'>
						<el-input v-model='editForm.id' :disabled='disb' class='ipt_sels'></el-input>
					</el-form-item>
					<el-form-item label='创建时间'>
						<el-input v-model='editForm.creation_time' :disabled='disb' class='ipt_sels'></el-input>
					</el-form-item>
					...
					<el-form-item label='A端'>
						<el-input  :disabled='disb'  class='ipt_sels'>
							<template slot-scope='scope'>
								<span>{{scope.row.a_node.name}}</span>-
								<span>{{scope.row.a_ip}}</span>-
								<span>{{scope.row.a_vlan}}</span>
							</template>
						</el-input>
					</el-form-item>
					<el-form-item label='A端描述'>
						<el-input v-model='editForm.a_desc' :disabled='disb' class='ipt_sels'></el-input>
					</el-form-item>
					<el-form-item label='Z端'>
						<el-input  :disabled='disb' class='ipt_sels'>
							<template slot-scope='scope'>
								<span>{{scope.row.z_node.name}}</span>-
								<span>{{scope.row.z_ip}}</span>-
								<span>{{scope.row.z_vlan}}</span>
							</template>
							
						</el-input>
					</el-form-item>
					<el-form-item label='Z端描述'>
						<el-input v-model='editForm.z_desc' :disabled='disb' class='ipt_sels'></el-input>
					</el-form-item>
    ...
    <section>
        ...
        >
          <template slot-scope='scope'>
            <el-tag
              ...
              @click='handleNode_a(scope.$index, scope.row)'
            >{{scope.row.a_node.name}}</el-tag>
            {{scope.row.a_ip}}-{{scope.row.a_vlan}}
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope='scope'>
            <el-tag
              ...
              type='primary'
              style='cursor: pointer;'
              @click='handleNode_z(scope.$index, scope.row)'
            >{{scope.row.z_node.name}}</el-tag>
            {{scope.row.z_ip}}-{{scope.row.z_vlan}}
          </template>
        </el-table-column>
        <el-table-column
      ...
      >
        ...
        >
          ...
          <el-form-item label='创建时间'>
            <el-input
              v-model='editForm.creation_time'
              :disabled='disb'
              class='ipt_sels'
            ></el-input>
          </el-form-item>
          ...
          <el-form-item label='总带宽'>
            <el-input
              ...
              :disabled='editFormStatue'
              class='ipt_sels'
            ></el-input>
          </el-form-item>
          <el-form-item label='物理带宽'>
            <el-input
              v-model='editForm.physical_bandwidth'
              :disabled='editFormStatue'
              class='ipt_sels'
            ></el-input>
          </el-form-item>
          ...
          >
            ...
            >
              <el-option
                v-for='(item,index) in detectionType'
                :value='item.value'
                :label='item.label'
                :key='index'
              >
          ...
          >
            <el-input
              v-model='editForm.monitoring_param'
              :disabled='editFormStatue'
            ></el-input>
          </el-form-item>
          <el-form-item label='流量获取键入值'>
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      this.$ajax
        .get("/link/links" + "?token=" + this.token, para)
        .then(res => {
          if (res.status == 200) {
            if (res.data.status == 0) {
              this.loading = false;
...
=======
          this.$ajax
            .del("/link/del_links" + "?token=" + this.token, para)
            .then(res => {
              console.log(res);
              if (res.status == 200) {
                if (res.data.status == 0) {
    ...
    handleStatus(index, row) {
      ...
      if (row.maintain_type == false) {
        this.$confirm("确认将此链路开启，开启维护状态!", "提示", {
          ...
          this.$ajax
            .put(
              "/link/open_maintenance_link/" + row.id + "?token=" + this.token
            )
            .then(res => {
              console.log(res);
              if (res.status == 200) {
        ...
        this.$confirm("确认将此链路关闭，关闭维护状态!", "提示", {
          ...
          this.$ajax
            .put(
              "/link/close_maintenance_link/" + row.id + "?token=" + this.token
            )
            .then(res => {
              console.log(res);
              if (res.status == 200) {
    ...
    handleNode_a(index, row) {
      this.$router.push({
        path: "/location/index/unknown/nodedetails/" + row.a_node.id
      });
    },
    //节点z的详情
    handleNode_z(index, row) {
      this.$router.push({
        path: "/location/index/unknown/nodedetails/" + row.z_node.id
      });
    },
    //详情
    handleSee(index, row) {
      ...
      this.$ajax
        .get("/link/link_info/" + row.id + "?token=" + this.token)
        .then(res => {
          console.log(res);
          if (res.status == 200) {
            if (res.data.status == 0) {
        ...
        if (valid) {
          this.$confirm("确认要提交吗？ ", "提示", {})
            .then(() => {
              ...
              this.$ajax
                .put(
                  "/link/edit_link/" + para.id + "?token=" + this.token,
                  para
                )
                .then(res => {
                  if (res.status == 200) {
    ...
    handleDel(index, row) {
      this.$confirm("确认要删除该数据吗？", "提示", {
        ...
        .then(() => {
          this.$ajax
            .del("/link/del_link/" + row.id + "?token=" + this.token)
            .then(res => {
              if (res.status == 200) {
                if (res.data.status == 0) {
                  this.$message({
...
>>>>>>> 7c9a1445ad4b1b5c7d70be3392d56f4ac45f789f
    ...
    exportData: function(params) {
      this.$ajax
        .get("/link/links" + "?token=" + this.token, params)
        .then(res => {
...
<<<<<<< HEAD
							this.$ajax.put('/link/edit_link/'+para.id+'?token='+this.token,para)
							.then( res => {
								if(res.status==200){
									this.editLoading=false;
									if(res.data.status==0){
