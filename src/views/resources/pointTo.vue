		...
		<div>
			...
			<section>
					...
					<el-col :span='24'>
						<el-form :model='filters' :inline='true' ref='filters'>
							<el-form-item :label='$t("Public.name")' prop='search_name'>
								<el-input v-model='filters.search_name' class='sel' :placeholder='$t("Public.name")'></el-input>
							</el-form-item>
							<el-form-item :label='$t("Public.tenant")' prop='search_tenant'>
								<el-select v-model='filters.search_tenant' filterable class='sel'>
									<el-option v-for='(item,index) in tenantData' :value='item.id' :label='item.name' :key='index'></el-option>
								</el-select>
							</el-form-item>
							<el-form-item :label='$t("Public.specialStatus")' prop='search_status'>
								<el-select v-model='filters.search_status' class='sel'>
									<el-option v-for='(item,index) in specialData' :label='item.label' :value='item.value' :key='index'></el-option>
								</el-select>
							</el-form-item>
							<el-form-item :label='$t("Public.specialType")' prop='search_type'>
								<el-select v-model='filters.search_type' class='sel'>
									<el-option v-for='(item,index) in specialTypeData' :label='item.label' :value='item.value' :key='index'></el-option>
								</el-select>
							</el-form-item>
							<el-form-item :label='$t("Public.bandwidth")' prop='search_start_bandwidth'>
								<el-input v-model='filters.search_start_bandwidth' class='ipts' :placeholder='$t("Public.origin")'></el-input>{{$t('Public.to')}}
							</el-form-item>
							<el-form-item prop='search_end_bandwidth'>
								<el-input v-model='filters.search_end_bandwidth' class='ipts' :placeholder='$t("Public.abort")'></el-input>
							</el-form-item>
							<el-form-item :label='$t("Public.billTime")' prop='billingTime'>
								<el-date-picker v-model="filters.billingTime" type="datetimerange" value-format="timestamp" :range-separator="$t('Public.to')" :start-placeholder="$t('Public.staBill')" :end-placeholder="$t('Public.endBill')" class='ipt'>
								</el-date-picker>
							</el-form-item>
							<el-form-item :label='$t("Public.expTime")' prop='overdueTime'>
								<el-date-picker v-model="filters.overdueTime" type="datetimerange" value-format="timestamp" :range-separator="$t('Public.to')" :start-placeholder="$t('Public.origin')" :end-placeholder="$t('Public.abort')" class='ipt'>
								</el-date-picker>
							</el-form-item>
							<el-form-item>
								<el-button size='small' @click='getUsers' type='primary'>{{$t('topFilters.search')}}</el-button>
				...
				<el-table :data="users" @selection-change="selsChange" highlight-current-row v-loading='getLoading'>
					<el-table-column type='selection' min-width='30' align='center' v-if='filProps'></el-table-column>
					<el-table-column type='index' :label='$t("Public.index")' min-width='30' align='center'>
						<template slot-scope='scope'>
							<span>{{scope.$index+(currentPage-1)*pagesize+1}}</span>
						</template>
					</el-table-column>
					<el-table-column :label='$t("Public.specialName")' min-width='60' align='center'>
						<template slot-scope='scope'>
							<span class="cli_spn" @click='handleSeePoint(scope.$index,scope.row)'>{{scope.row.name}}</span>
						</template>
					</el-table-column>
					<el-table-column :label='$t("Public.specialStatus")' min-width='60' align='center'>
						<template slot-scope='scope'>
							<span :class="scope.row.statusColor">{{scope.row.statusHTML}}</span>
						</template>
					</el-table-column>
					<el-table-column prop='bandwidth' :label='$t("Public.bandwidth")' min-width='50' align='center'>
					</el-table-column>
					<el-table-column :label='$t("Public.specialType")' min-width='50' align='center'>
						<template slot-scope='scope'>
					...
					<el-table-column :label='$t("Public.zPort")' min-width='60' align='center'>
						<template slot-scope='scope'>
					...
					<el-table-column prop='creation_time' :label='$t("Public.creation")' width='80' align='center'>
					</el-table-column>
					<el-table-column prop='charge_time' :label='$t("Public.billTime")' width='80' align='center'>
					</el-table-column>
					<el-table-column prop='expiration_time' :label='$t("Public.expTime")' width='80' align='center'>
					</el-table-column>
					<el-table-column prop='tenant.name' :label='$t("Public.tenant")' min-width='70' align='center'>
					</el-table-column>
					<el-table-column :label='$t("Public.description")' min-width='70' align='center'>
						<template slot-scope='scope'>
							{{ scope.row.description | descriptionValue }}
						</template>
					</el-table-column>
					<el-table-column :label='$t("Public.operation")' width='140' align='center' v-if='filProps'>
						<template slot-scope='scope' v-if='scope.row.status == "creating"?false:true '>
							<el-button size='mini' type='warning' v-if='buttonVal.stop? scope.row.status !=="failure"?true:false  : buttonVal.stop' @click='handleStatus(scope.$index,scope.row)'>{{scope.row.specialName}}</el-button>
							<el-button size='mini' type='primary' @click='handleEdit(scope.$index,scope.row)' v-if='buttonVal.edit ? scope.row.status !=="failure"?true:false :buttonVal.edit'>{{$t('tabOperation.edit')}}</el-button>
							<el-button size='mini' type='danger' @click='handleDel(scope.$index,scope.row)' v-if='buttonVal.del ? scope.row.status=="failure"?true:false : buttonVal.del'>{{$t('tabOperation.delete')}}</el-button>
				...
				<el-row v-if='filProps' class='toolbar'>
					<el-col :span='24'>
						<el-pagination :total="total" @size-change="handleSizeChange" @current-change="handleCurrentChange" layout="total, sizes, prev, pager, next, jumper" :page-sizes="[10, 20, 30,50]" :page-size='pagesize' :current-page.sync="currentPage" :page-count='pageNum' :pager-count="pagecount"></el-pagination>
				...
				<el-dialog :title='textMap[dialogStatus]' :visible.sync='dialogFormVisible' :close-on-click-modal='false' v-loading='editLoading'>
					<el-form :model='editForm' ref='editForm' label-width='200px' :rules='editFormRules'>
						<el-form-item :label='$t("Public.specialName")' prop='name'>
						...
						<el-form-item :label='$t("Public.busiStatus")'>
							<el-input v-model='editForm.statusHTML' class='ipt' disabled></el-input>
						</el-form-item>
						...
						<el-form-item :label='$t("Public.billTime")'>
							...
							<el-date-picker v-model="editForm.charge_time" format="yyyy-MM-dd HH:mm:ss" value-format="timestamp" clearable style="width: 100%" :picker-options="startDatePicker" type="datetime" class='ipt' :placeholder="$t('Public.plaStartBill')">
							</el-date-picker>
						</el-form-item>
						<el-form-item :label='$t("Public.expTime")'>
							<!--<el-input v-model='editForm.expiration_time'></el-input>-->
							<el-date-picker v-model="editForm.expiration_time" format="yyyy-MM-dd HH:mm:ss" value-format="timestamp" clearable style="width: 100%" :picker-options="endDatePicker" type="datetime" class='ipt' :placeholder="$t('Public.plaEndBill')">
							</el-date-picker>
						</el-form-item>
					...
					<div slot='footer' class='dialog-footer'>
						...
						<!--编辑-->
						<el-button v-if="dialogStatus=='update'" size='small' type="primary" @click="UpdateData">{{$t('tabOperation.save')}}</el-button>
					</div>
				</el-dialog>
			</section>
		</div>
		...
		export default {
			...
			methods: {
				...
				getUsers() {
					...
					let para = {
						search_tenant: typeof this.customerID !== "undefined" ? this.customerID : this.filters.search_tenant,
