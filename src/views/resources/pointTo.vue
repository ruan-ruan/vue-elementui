				...
				<el-col :span='24'>
					<el-form :model='filters' :inline='true' ref='filters' >
						...
						<el-form-item label='租户标识'>
							<el-select v-model='filters.search_tenant' class='sel'>
						...
						<el-form-item label='专线状态'>
							<el-select v-model='filters.search_status' class='sel'>
								<el-option v-for='(item,index) in specialData'
									:label='item.label'
									:value='item.value'
									:key='index'></el-option>
							</el-select>
						...
						<el-form-item label='专线类型'>
							<el-select v-model='filters.search_type' class='sel'>
								<el-option v-for='(item,index) in specialTypeData'
									:label='item.label'
									:value='item.value'
									:key='index'></el-option>
							</el-select>
						...
						<el-form-item label='带宽(Mbps)'>
							...
							<el-input v-model='filters.search_end_bandwidth' class='ipts'placeholder='截止'></el-input>
						</el-form-item>
						<el-form-item label='计费时间'>
							    <el-date-picker
							      ...
							      end-placeholder="结束计费日期时间" class='ipt'>
							    </el-date-picker>
						</el-form-item>
			...
			<el-table :data = "users" @selection-change="selsChange" highlight-current-row style='width: 100%; ' v-loading='getLoading'>
				...
				<el-table-column  label='操作' width='260'  align='center' v-if='parentStatus'>
					<template slot-scope='scope'>
						...
						<el-button size='small' type='danger' @click='handleDel(scope.$index,scope.row)' v-if='scope.row.creat'>删除</el-button>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	export default{
		...
		created(){
			...
			if(typeof this.customer !=='undefined' || typeof this.nodeID !=='undefined'|| typeof this.customerID !=='undefined'){
...
//			})
		...
		methods:{
			...
			getUsers(){
				//获取所有的点的数据
				let para={
					search_tenant:(typeof this.customerID !=='undefined' ?this.customerID:''),
...
//		    	console.log('进入专线详情');
		    	...
		    	this.$refs.editForm.validate(valid => {
		    		if(valid) {
		    			...
		    			.then(() => {
		    				...
		    				this.$ajax.put('/vll/edit_p2p_vll/'+this.editForm.id+'?token='+this.token,para)
		    				.then(res => {
		    					if(res.status==200){
		    						if(res.data.status==0){
		    							this.$message({
		    ...
		    handleStatus(index,row){
		    	...
		    	if(row.status=='停止中'){
		    		...
		    		.then( () => {
		    			this.$ajax.put('/vll/to_server_p2p_vll/'+row.id+'?token='+this.token)
		    			.then(res => {
		    				if(res.status==200){
		    					if(res.data.status==0){
		    						this.$message({
		    		...
		    		.then(() => {
		    			...
		    			this.$ajax.put('/vll/to_stop_p2p_vll/'+row.id+'?token='+this.token)
		    			.then(res => {
		    				if(res.status==200){
		    					if(res.data.status==0){
		    						this.$message({
			...
			exportData:function(params){
				...
				this.$ajax.get('/vll/p2p_vlls'+'?token='+this.token,params)
				.then(res => {
					if(res.status==200){
						if(res.data.status==0){
			...
			beginDate(){
	            ...
	            return {
	            disabledDate(time){
	                if (self.editForm.expiration_time) {  //如果结束时间不为空，则小于结束时间
	                    return new Date(self.editForm.expiration_time).getTime() < time.getTime()
	                } else {
	       	...
	       	processDate() {
	            ...
	            return {
	                disabledDate(time) {
	                if (self.editForm.charge_time) {  //如果开始时间不为空，则结束时间大于开始时间
	                    return new Date(self.editForm.charge_time).getTime() > time.getTime()
	                } else {
	                    // return time.getTime() > Date.now()//开始时间不选时，结束时间最大值小于等于当天
	                }
	                }
